# Migrating Expo UI to Planton Design System

Read `artifacts/planton-ds/docs/AGENTS.md` and
`artifacts/planton-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/planton-ds/lib/native-theme` and
  `@workspace/planton-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/planton-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/planton-ds` and retains only
